...
def _get_start_node(event: Event) -> str | None:
    ...
    Preference order: component > supplier > factory.
    """
...
def _estimate_inventory_buffer_days(
    session: Session,
    component_id: int,
    factory_id: int | None,
    ...
    query = session.query(Inventory).filter_by(
        item_type="component",
...
def analyze_impact(
    ...
    """
    Analyze the downstream impact of an understood event.

    If a dependency graph is supplied, reuse it. Otherwise build one.
    """

    if event.status != "understood":
        raise ValueError(
            "Event must be understood before impact analysis can run."
        )

    start_node = _get_start_node(event)

    if start_node is None:
        # Entity linking failed in Phase 3
        return None

    delay_days, method = estimate_source_delay_days(
        event.raw_text,
        event.event_type,
        event.severity,
    )

    # Reuse graph if provided; otherwise build it.
    ...
    report.affected_component_names = [
        n["name"] for n in downstream.get("component", [])
    ]

    report.affected_product_names = [
        n["name"] for n in downstream.get("product", [])
    ]

    report.affected_factory_names = [
        n["name"] for n in downstream.get("factory", [])
    ]

    affected_product_db_ids = {
        n["db_id"] for n in downstream.get("product", [])
    }

    affected_po_db_ids = {
        n["db_id"] for n in downstream.get("purchase_order", [])
    }
...
def _compute_overall_severity(report: ImpactReport) -> str:
    max_delay = max(
        (po.delay_days for po in report.affected_pos),
        default=0,
    )

    return compute_severity_from_delay_and_priority(
        max_delay,
        report.high_priority_customers_affected,
    )